...
> [!IMPORTANT]
> When using C3 to create this project, select "no" when it asks if you want to deploy. You need to follow this project's [setup steps](https://github.com/cloudflare/templates/tree/main/purple-credit-1483#setup-steps) before deploying.
...
```bash
npm create cloudflare@latest -- --template=cloudflare/templates/purple-credit-1483
```

A live public deployment of this template is available at [https://purple-credit-1483.templates.workers.dev](https://purple-credit-1483.templates.workers.dev)
...
1. Install the project dependencies with a package manager of your choice:
   ...
   npm install
   ```
2. Create a [D1 database](https://developers.cloudflare.com/d1/get-started/) with the name "summer-water-78b3-d1":
   ```bash
   npx wrangler d1 create summer-water-78b3-d1
   ```
   ...and update the `database_id` field in `wrangler.json` with the new database ID.
3. Run the following db migration to initialize the database (notice the `migrations` directory in this project):
   ```bash
   npx wrangler d1 migrations apply --remote summer-water-78b3-d1
   ```
